...
continuing line must drop straight.  A TB section draws each line at the rotation
image of its stored offset (``station.x - offset``), constant along a collinear
run, so the continuation keeps one X from hub to child and drops straight by
construction; the same constant-offset draw keeps a collinear fan-in feeder on
its lane into a terminal merge.
